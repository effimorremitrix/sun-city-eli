...
export function Team({ agents = [], variant = "primary" }: Props) {
  ...
  if (useDb) {
    const agentCard = (a: PublicAgentRow) => {
      ...
      return (
        <article className="soft-card h-full p-5 text-center transition-transform hover:-translate-y-1">
          ...
          {a.role_title && <p className="mt-1 text-sm font-semibold text-sun">{a.role_title}</p>}

          <div className="mt-4 flex gap-2">
            <a
              {...waProps(t.team.waAgent(name, business.name), a.phone_tel ?? undefined)}
        ...
        <div className="mx-auto max-w-6xl px-4">
          ...
          {variant === "secondary" ? (
            ...
            <ul className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {dbAgents.map((a, i) => (
                <li key={a.id}>
                  <Reveal delay={i * 70}>{agentCard(a)}</Reveal>
                </li>
              ))}
            </ul>
          )}
