...
- **מאגר השוק (`market_listings`)** — מודעות מהלוחות (יד2, קומו; מדלן/הומלס/וין
  ...
  **ללקוחות מוצגות רק מודעות של משרדי תיווך** (`advertiser_type = 'agency'`):
  יד2 לפי שם המשרד/הדלי בפיד, קומו לפי סימן "תיווך"/"פרטי" בכרטיס
  (`classifyKomoAdvertiser`), ולוחות ה-AI לפי דיווח המודל. מודעה פרטית או
  מודעה שלא הוכרע מי פרסם אותה (`unknown`) נשמרת במאגר אך אינה מוצגת ואינה
  נשלחת בהתראות; טאב "שוק" מציג את הסיווג של כל מודעה.
...
### CRM: משימות, אחראי וכסף

הפייפליין שייך לדף (`site`): כל סוכן מנהל את הלידים והמשימות שלו (RLS דרך
`owns_site`), והמנהל הראשי רואה את כל הצוות באותו מסך.

- **`lead_tasks`** — כמה משימות פתוחות לכל ליד (כותרת, סוג, מועד יעד, הערות,
  אחראי). המסד מסנכרן אוטומטית את המשימה הפתוחה הקרובה חזרה ל-
  `leads.next_follow_up_at` / `next_action` (`sync_lead_next_task`), ולכן דליי
  ה-Follow-up ומונה תשומת הלב ממשיכים לעבוד כמו קודם.
- **`leads.assigned_user_id`** — האחראי בתוך הדף (ברירת מחדל: בעל הדף, נקבע
  בטריגר בכל נתיב יצירה). שדה אחריות, לא גבול הרשאות.
- **`leads.deal_value` / `lost_reason` / `closed_at`** — כסף, סיבת אובדן מרשימה
  סגורה (`src/lib/crm.ts`) וחותמת סגירה אוטומטית.
- **לוח הפייפליין** (`crm_pipeline_board`) — תמונת מצב (פתוחים, לא טופלו,
  משימות באיחור, שקטים 14 יום) לצד תוצאה בטווח תאריכים (עסקאות, ערך, אובדן).
- **תזכורות** — המשימה `follow-up-reminders` שולחת לסוכן מייל (ווואטסאפ אם
  התבנית `WA_TEMPLATE_AGENT_REMINDER` מוגדרת) כשמועד המשימה מגיע, לפי חותמות
  ה-Outbox `reminder_email_sent_at` / `reminder_whatsapp_sent_at`. הזזת המועד
  מאפסת אותן, ולכן דחייה מייצרת תזכורת חדשה ולא כפילות.
...
- **הגדרות** (`app_settings`): מכסות חיפוש חכם (לגולש אנונימי / למשתמש / לדקה),
  ...
  חסימה אוטומטית, ריטנשן גיבויים, תזכורות Follow-up.
...
| `follow-up-reminders` | :00/:30 בין 06–17     | תזכורת לסוכן על משימות ולידים שהגיע מועדם                        |
...
### ממליצים ו"מהשטח" (שלב 2)

- **ממליצים** (`testimonials`): טבלה עם היקף הצגה — `global` (כל הדפים) או `sites`
  (דף אחד או כמה, `site_ids`). מוזנים פעם אחת בטאב "ממליצים"; ההמלצות הישנות
  מ-`site_content.testimonials` הועברו אוטומטית בהיקף "הדף שבו נשמרו", ואפשר
  להפוך כל אחת לכללית בלחיצה. תמיכה בטקסט / תמונה / סרטון, תרגום אוטומטי.
- **מהשטח** (`field_media`): סרטונים ותמונות מעסקאות (חתימות, מסירת מפתחות,
  לקוחות מרוצים, המשרד) עם אותו מודל היקף, מדור "מהשטח" באתר וטאב ניהול.

### סטטיסטיקות ומשפך (שלב 2)

`analytics_funnel(p_from, p_to)`: כניסות → צפייה בנכס → פנייה → הרשמה → ליד →
התעניינות → עסקה, לכלל האתר ולכל סוכן; מקורות הלידים (utm / referrer / דף
סוכן / קמפיין) ומה כל מקור מייצר (לידים, התעניינויות, עסקאות). לחיצות וואטסאפ
וטלפון נמדדות במאזין אחד ברמת המסמך (`installClickTracking`) על כל קישור
`tel:` / `wa.me`; ההרשמה נמדדת בשרת עם הסוכן של הלקוח.
