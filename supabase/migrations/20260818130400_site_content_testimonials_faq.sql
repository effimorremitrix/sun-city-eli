-- תוכן אתר: ממליצים (כולל סרטוני המלצה) ושאלות נפוצות הניתנים לעריכה מהאזור האישי.
-- NULL בעמודה = נפילה חזרה לתוכן הסטטי במילוני ה-i18n (התנהגות קיימת נשמרת).
--   testimonials: [{"id","name","type","quote","videoUrl"?}]
--   faq:          [{"id","q","a"}]
ALTER TABLE public.site_content
  ADD COLUMN IF NOT EXISTS testimonials jsonb,
  ADD COLUMN IF NOT EXISTS faq jsonb;

-- עדכון הפונקציה הציבורית כך שתחזיר גם ממליצים ושאלות נפוצות
-- (העתק מדויק של הגוף מ-20260817131034 + שני השדות החדשים).
