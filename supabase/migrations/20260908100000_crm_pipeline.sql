...
-- מלמעלה בלוח הצוות. אין ישות הרשאות חדשה: מי שמנהל את הדף מנהל את
-- הפייפליין שלו, בדיוק כמו בנכסים ובלידים.
-- ============================================================

-- ---------- 1. שדות הפייפליין על הליד ----------
...
COMMENT ON COLUMN public.leads.deal_value IS 'ערך העסקה בשקלים (מחיר הנכס או העמלה, לפי החלטת המשרד) — למדידת כסף ולא רק ספירות.';
COMMENT ON COLUMN public.leads.lost_reason IS 'סיבת אובדן כשהסטטוס הוא "לא רלוונטי". רשימת הערכים היא בקוד (src/lib/crm.ts), כמו status ו-source.';

-- ייחוס אחורה: האחראי הוא בעל הדף, וחותמת הסגירה של לידים סגורים היא
-- העדכון האחרון שלהם. שינוי לא הרסני — שום נתון קיים לא מוחלף.
...
-- ברירות המחדל של ה-CRM על הליד, בטריגר אחד כדי שכל נתיב יצירה (טופס
-- ציבורי, webhook, אזור אישי, הזנה ידנית) יקבל אותן:
--  * חותמת סגירה נכתבת כשהליד נסגר ומתאפסת אם הוא נפתח מחדש
--  * האחראי הוא בעל הדף, וליד שהועבר לדף אחר עובר לבעליו
...
-- ---------- 2. משימות הליד ----------
-- ליד אחד, כמה משימות פתוחות. next_follow_up_at על הליד נשאר "המשימה
-- הבאה" לתצוגה המהירה, והוא מסונכרן אוטומטית מהמשימה הפתוחה הקרובה.
...
-- אינדקס ה-Outbox: המשימות שהתזכורת עליהן עוד לא נשלחה
...
-- חותמת השלמה + איפוס ה-Outbox כשהמועד זז
...
-- העברת ליד לסוכן אחר גוררת את המשימות שלו, אחרת הן נופלות מחוץ ל-RLS
-- של שני הצדדים ונעלמות מהמסך.
...
-- "המשימה הבאה" על הליד = המשימה הפתוחה הקרובה ביותר. מסונכרן מהמסד כדי
-- שכל מסך שכבר קורא next_follow_up_at (דליי ה-Follow-up, לוח הבקרה,
-- מונה תשומת הלב) ימשיך לעבוד בלי שינוי.
...
BEGIN
  ...
  -- לליד בלי משימות בכלל לא נוגעים: ה-Follow-up הידני שלו נשאר כפי שהוא
...
AS $$
BEGIN
  -- ב-DELETE אין NEW בכלל (גישה אליו זורקת), ולכן מפצלים לפי TG_OP
...
-- ---------- 3. לוח הצוות ----------
-- מבט אחד על הפייפליין: מצב נוכחי (לידים פתוחים, משימות באיחור) לצד תוצאה
-- בטווח התאריכים (עסקאות, כסף, אובדן וסיבותיו). הסוכן מקבל את הדף שלו,
-- המנהל הראשי את כל הצוות — אותה פונקציה, אותם מספרים.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- הפייפליין לפי שלב, בכל היקף הצפייה
    'byStatus', (
    ...
    -- למה לידים נסגרו בלי עסקה — התשובה שמכוונת את המשרד
    'lostReasons', (
    ...
    -- העסקאות שנסגרו בטווח, לפי גודל (ה-LIMIT חייב להיות בתוך התת-שאילתה,
    -- אחרת הוא חל על שורת האגרגציה היחידה ולא על העסקאות)
    'closedDeals', (
...
-- מי אפשר להציב כאחראי על ליד בדף מסוים: בעל הדף וכל מנהל ראשי.
-- מודל ההרשאות לא משתנה, זו רק רשימת בחירה נכונה במסך.
